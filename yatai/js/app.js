...
(function (YH) {
  ...
  function sheets() {
    return [YH.SHEET_FRONT, YH.SHEET_GUIDE, YH.SHEET_BACK].filter(Boolean);
  }
  ...
  function overflowOf(sh) {
    ...
    sh.querySelectorAll('.cell, .sec-head, .line-head, .box').forEach(function (n) {
    ...
    /* 枠自体は用紙の中にあっても、文章が長いと中身だけが枠から溢れて
       ...
       ★ scrollHeight は使えない。overflow: visible の要素では
         ...
         もとの checkOverflow はこれで、枠あふれを検知できていなかった。
       ★ 枠の中の .box-body は flex アイテムだが min-height: auto なので
         中身より小さくならない。つまりあふれるのは .box-body 自身で、
         .box-body の中を見ても見つからない。枠(.box)を基準に、
         中の要素すべての rect のいちばん下を取る。 */
  ...
  /* ---- 用紙に収まる大きさに合わせる ----
     同じ 4.0mm でも、機械のフォントや Chrome の文字設定で列の幅・長さが
  ...
  function wireToolbar() {
    ...
    $('#btn-reset-text').addEventListener('click', function () {
      if (!confirm('2枚目の「たたくときのポイント」の文章を、最初の文にもどします。よろしいですか？')) return;
      YH.resetTexts();
      build();
      fitAll();
      checkOverflow();
